...
const heroMeta = [
  { label: 'Current conversations', value: 'Faith, family, purpose' },
  { label: 'Primary platform', value: 'YouTube-first storytelling' },
  { label: 'Built for', value: 'Christ-centered growth' },
];

export default function HeroSection() {
  return (
    <PageHero
      eyebrow="Editorial podcast home"
      title="A more thoughtful home for the message of The Higher Life."
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          {heroMeta.map((item) => (
            <div key={item.label} className="rounded-[1.25rem] border border-white/10 bg-white/5 p-4">
              <p className="text-[0.7rem] font-semibold uppercase tracking-[0.18em] text-gold-300">
                {item.label}
              </p>
